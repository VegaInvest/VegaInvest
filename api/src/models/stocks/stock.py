...
class Stock(object):
    ...
    def update_rawData():
        ...
        data = yf.download(StockConstants.TICKERS, start=date, end=date,interval="1d")["Adj Close"]
        data.columns.name=""
    ...
    def push_rawData(start_date, end_date):
        data = yf.download(StockConstants.TICKERS, start=start_date, end=end_date,interval="1d")["Adj Close"]
        data.columns.name=""
        ...
        data.columns = data.columns.astype(str)
        Database.initialize()
        Database.insert_many("rawdata",data.to_dict('record'))
    ...
    def get_from_db(startdate,enddate):
        ...
        #print(list(blah)[:])
        datab=pd.DataFrame.from_dict(results, orient='columns', dtype=None, columns=None)
